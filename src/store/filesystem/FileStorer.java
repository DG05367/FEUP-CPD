...
public class FileStorer extends NodeFileHandler {
    ...
    public FileStorer(NodeState nodeState) throws IOException {
        super(nodeState);
        this.fileFolder = "persistent-storage/" + nodeState.getNodeId() + "/files";
        this.build();
    }
    ...
    public void distributeFiles() throws IOException {
        ...
        for (File file : files) {
            ...
            PutRequest request = new PutRequest(keyGen(new FileInputStream(filePath)), filePath);
            Socket neighbourNode = new Socket(nearestNodeId, getNodeState().getTcpConnectionAddress().getPort());
