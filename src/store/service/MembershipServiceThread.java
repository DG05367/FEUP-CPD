...
public class MembershipServiceThread extends ServiceThread {
    ...
    @Override
    public void run() {
        System.out.println("MEMBERSHIP SERVICE: Start\nListening to port " + nodeState.getMultiCastPort());
        try {
            MulticastSocket socket = new MulticastSocket(nodeState.getMultiCastPort());
            socket.joinGroup(nodeState.getMultiCastIpAddress());
